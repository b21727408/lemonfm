...
| E5 | Select and run a dependency-vulnerability audit for the resolved Gradle and Pub graphs. Verify direct/transitive coverage, report unsupported portions and establish finding triage using existing tools; no custom scanning platform. Quality owns the required execution points. | Once dependency resolution exists, before bootstrap acceptance; repeat on dependency changes and before an external pilot | Engineering |
